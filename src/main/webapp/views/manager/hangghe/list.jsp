	...
	<div>
		<div class="d-flex">
			...
			<div class="flex-fill">
				...
				<div class="py-2 px-3">
					<h5 class="text-dark fs-6 my-3">Trang chủ / Quản lý hàng ghế</h5>
					<div class="rounded-3 overflow-hidden shadow-sm bg-white px-4 py-3">
						<div class="d-flex justify-content-between">
							<h5 class="mb-3">Danh sách hàng ghế</h5>
							<div>
								<a href="./themmoicombo.html"
									...
									href="/admin/createhangghe"> <i class="ri-file-add-line"></i>
									Thêm mới
								</a>
							</div>
						</div>
							...
							<tbody>
								<c:forEach items="${list}" var="item" varStatus="loop">
									<tr class="text-sm">
										...
										<td>${loop.index+1}</td>
										<td>${item.maHangGhe}</td>
										<td>${item.tenHangGhe}</td>
										<td>
											<div class="d-flex column-gap-2">
												<a
													class="ri-delete-bin-6-line text-decoration-none text-black-50"
													style="cursor: pointer" title="Xóa"
													href="/admin/deletehangghe?id=${item.maHangGhe}"></a><a
													href="/admin/updatehangghe?id=${item.maHangGhe}"
