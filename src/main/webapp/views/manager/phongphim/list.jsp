					...
					<div class="rounded-3 overflow-hidden shadow-sm bg-white px-4 py-3">
						<div class="d-flex justify-content-between">
							...
							<div>
								...
								</a> <a href="/admin/createphongphim"
							...
							<tbody>
								<c:forEach items="${list}" var="item" varStatus="loop">
									<tr class="text-sm">
										...
										<td>${loop.index +1}</td>
										<td>${item.maPhongPhim}</td>
										<td>${item.tenPhongPhim}</td>
										<td><div
												class="text-white ${item.trangThai ? 'bg-success px-2 py-1' : 'bg-danger px-2 py-1'}">${item.trangThai ? 'Đang hoạt động' : 'Ngừng hoạt động'}</div></td>
										<td>${item.rapPhim.tenRapPhim}</td>
										<td>${item.ghes.size()}</td>
										<td><div class="d-flex column-gap-2">
												<a href="/admin/deletephongphim?id=${item.maPhongPhim}"
													class="ri-delete-bin-6-line text-decoration-none text-black-50"
													style="cursor: pointer" title="Xóa"></a><a
													href="/admin/updatephongphim?id=${item.maPhongPhim}"
